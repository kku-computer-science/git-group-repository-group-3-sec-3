...
${PERMISSION_MENU}    xpath=//a[contains(@class,'nav-link') and .//span[contains(@class,'menu-title') and (contains(text(),'Permissions') or contains(text(),'สิทธิ์') or contains(text(),'权限'))]]

# ปุ่ม 
${ADD_PERMISSIONS_BUTTON}    xpath=//div[contains(@class,'card-header')]//a[contains(@href, '/permissions/create')]
${VIEW_PERMISSIONS_BUTTON}    xpath=//div[contains(@class,'card-body')]//a[contains(@class, 'btn-outline-primary') and .//i[contains(@class, 'mdi-eye')]]
${EDIT_PERMISSIONS_BUTTON}    xpath=//div[contains(@class,'card-body')]//a[contains(@class, 'btn-outline-success') and .//i[contains(@class, 'mdi-pencil')]]
...
@{EXPECTED_ADD_PERMISSIONS_PAGE_EN}    
...    Create Permission
...    Name  
...    Permission                     
...    Submit            
@{EXPECTED_ADD_PERMISSION_PAGE_CN}    
...    创建角色    名字    允许:
...    提交        
...
@{EXPECTED_EDIT_EN}    
...    Edit Permission    Name    Permissions                
...    Submit    
...
Go To Permissions
    Wait Until Element Is Visible    ${PERMISSION_MENU}    timeout=10s
    Click Element    ${PERMISSION_MENU}
...
Go To Add Permission
    Scroll Element Into View    ${ADD_PERMISSIONS_BUTTON}
    Wait Until Element Is Visible    ${ADD_PERMISSIONS_BUTTON}    timeout=15s
    Log    ${ADD_PERMISSIONS_BUTTON}
    Log Source
    Click Element    ${ADD_PERMISSIONS_BUTTON}
    Sleep    2s

Go To VIEW Permission
    Scroll Element Into View    ${VIEW_PERMISSIONS_BUTTON}
    Wait Until Element Is Visible    ${VIEW_PERMISSIONS_BUTTON}    timeout=15s
    Log    ${VIEW_PERMISSIONS_BUTTON}
    Log Source
    Click Element    ${VIEW_PERMISSIONS_BUTTON}
    Sleep    2s

Go To EDIT Permission
    Scroll Element Into View    ${EDIT_PERMISSIONS_BUTTON}
    Wait Until Element Is Visible    ${EDIT_PERMISSIONS_BUTTON}    timeout=15s
    Log    ${EDIT_PERMISSIONS_BUTTON}
    Log Source
    Click Element    ${EDIT_PERMISSIONS_BUTTON}
    Sleep    2s
...
Click Add Fund And Verify
    Sleep    3s
    Page Should Contain Element    ${ADD_PERMISSIONS_BUTTON}
