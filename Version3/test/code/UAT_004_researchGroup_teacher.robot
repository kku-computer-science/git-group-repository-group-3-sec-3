...
...    ข้อมูลกลุ่มวิจัย   
...    ชื่อกลุ่มวิจัย (ไทย)      
...    ชื่อกลุ่มวิจัย (อังกฤษ)   
...    คำอธิบายกลุ่มวิจัย (ไทย)  
...    คำอธิบายกลุ่มวิจัย (อังกฤษ)
...    รายละเอียดกลุ่มวิจัย (ไทย)     
...    รายละเอียดกลุ่มวิจัย (อังกฤษ)  
...    หัวหน้ากลุ่มวิจัย    ผศ.ดร. พุธษดี ศิริแสงตระกูล
...    สมาชิกกลุ่มวิจัย    พงษ์ศธร จันทรย้อย     
...    ย้อนกลับ
...
...    Research Group Details (Thai)     
...    Research Group Details (English)   
...    Research Group Leader    Asst. Prof. Dr. Pusadee Seresangtakul 
...    Research Group Members    Pongsathon Janyoi      
...    Back
...
Go To VIEW Research Group
    Scroll Element Into View    ${VIEW_RESEARCH_GROUP_BUTTON}
    Wait Until Element Is Visible    ${VIEW_RESEARCH_GROUP_BUTTON}    timeout=15s
    Log    ${VIEW_RESEARCH_GROUP_BUTTON}
    Log Source
    Click Element    ${VIEW_RESEARCH_GROUP_BUTTON}
    Sleep    2s

Go To EDIT Research Group
    Scroll Element Into View    ${EDIT_RESEARCH_GROUP_BUTTON}
    Wait Until Element Is Visible    ${EDIT_RESEARCH_GROUP_BUTTON}    timeout=15s
    Log    ${EDIT_RESEARCH_GROUP_BUTTON}
    Log Source
    Click Element    ${EDIT_RESEARCH_GROUP_BUTTON}
    Sleep    2s

Delete button
    Scroll Element Into View    ${DELETE_RESEARCH_GROUP_BUTTON}
    Wait Until Element Is Visible    ${DELETE_RESEARCH_GROUP_BUTTON}    timeout=10s
    Click Element    ${DELETE_RESEARCH_GROUP_BUTTON}
