...
<div class="container">
    ...
    <div class="col-md-8 grid-margin stretch-card">
        <div class="card" style="padding: 16px;">
            <div class="card-body">
                <h4 class="card-title">{{ trans('dashboard.Edit Funds') }}</h4></h4>
                <p class="card-description">{{ trans('dashboard.Fill in the information to edit research fund details.') }}</p>
                <form class="forms-sample" action="{{ route('funds.update',$fund->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group row">
                        <p class="col-sm-3 "><b>{{ trans('dashboard.Type Funds') }}</b></p>
                    ...
                    <div id="fund_code">
                        <div class="form-group row">
                            <p class="col-sm-3"><b>{{ trans('dashboard.Level Funds') }}</b></p>
                            <div class="col-sm-4">
                                <select name="fund_level" class="custom-select my-select">
                                    <option value=""{{ $fund->fund_level == '' ? 'selected' : '' }}>{{ trans('dashboard.Not specified') }}</option>
                                    <option value="สูง" {{ $fund->fund_level == 'สูง' ? 'selected' : '' }}>{{ trans('dashboard.High') }}</option>
                                    <option value="กลาง" {{ $fund->fund_level == 'กลาง' ? 'selected' : '' }}>{{ trans('dashboard.Medium') }}</option>
                                    <option value="ล่าง" {{ $fund->fund_level == 'ล่าง' ? 'selected' : '' }}>{{ trans('dashboard.Low') }}</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="form-group row">
                        <p class="col-sm-3 "><b>{{ trans('dashboard.Funds Name') }}</b></p>
                        <div class="col-sm-8">
                            <input type="text" name="fund_name" value="{{ $fund->fund_name }}" class="form-control" placeholder="fund_name">
                        </div>
                    </div>
                    <div class="form-group row">
                        <p class="col-sm-3 "><b>{{ trans('dashboard.Supporting Agencies Research Projects') }}</b></p>
                        <div class="col-sm-8">
                    ...
                    <a class="btn btn-light mt-5" href="{{ route('funds.index')}}">{{ trans('dashboard.Cancel') }}</a>
                </form>
            </div>
        </div>
    </div>
